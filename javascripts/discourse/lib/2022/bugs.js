...
let BugDispatch = {
  ...
  initialize: function (options) {
    ...
    if (this.options.canInteract) {
      setInterval(function () {
        that.check_if_spider_close_to_bug();
      }, 100);
    }
  ...
  near_bug: function (bug, action) {
    this.on_bug(bug, action);
  },

  on_bug: function (bug, action) {
    if (!bug.alive) {
      return;
    }
...
export { BugController, SpiderController };
